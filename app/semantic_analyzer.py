...
class SemanticAnalyzer:
    ...
    def visit_class_decl(self, node: ClassDecl):
        """Visita declaración de clase"""
        # Verificar que no exista
        existing = self.symbol_table.lookup_local(node.name)
        if existing:
            self._add_error(
                node.line, node.col,
                f"Clase '{node.name}' ya declarada en línea {existing.line}"
            )
            return
        ...
        symbol = Symbol(
            name=node.name,
            kind=SymbolKind.VARIABLE,  # O podrías crear SymbolKind.CLASS
            data_type=DataType.UNKNOWN,
            line=node.line,
            col=node.col,
            initialized=True
    ...
    def visit_new_expr(self, node: NewExpr) -> DataType:
        """Visita expresión new"""
        # Verificar que la clase existe
        symbol = self.symbol_table.lookup(node.class_name)
        if not symbol:
            self._add_error(
                node.line, node.col,
                f"Clase '{node.class_name}' no declarada"
            )
            return DataType.ERROR
        ...
        # Marcar como usada
        self.symbol_table.mark_used(node.class_name)

        return DataType.UNKNOWN  # O podrías retornar un tipo específico para instancias
